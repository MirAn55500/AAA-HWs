...
import asyncio
from telegram import InlineKeyboardButton, Update, Message, User, Chat, CallbackQuery
from telegram.ext import ContextTypes, Application

from tg_bot import get_default_state, generate_keyboard, start, initial_step, won, game
...
def test_generate_keyboard():
    ...
    expected = [[InlineKeyboardButton(state[r][c], callback_data=f'{r}{c}') for r in range(3)] for c in range(3)]
    assert actual == expected
...
async def test_start(mocker, get_update_context):
    ...
    assert actual == 0
    assert context.user_data['keyboard_state'] == [[FREE_SPACE for _ in range(3)] for _ in range(3)]


@pytest.mark.asyncio
async def test_initial_step_cross(mocker, get_update_context):
    update, context = get_update_context
    query = CallbackQuery(data=CROSS, id=1, from_user=update.message.from_user, chat_instance='', message=update.message)
    update = Update(1, message=update.message, callback_query=query)
    context.user_data['keyboard_state'] = [[FREE_SPACE for _ in range(3)] for _ in range(3)]
    mock_reply_text = mocker.patch.object(Message, 'reply_text', return_value=None)
...
async def test_initial_step_zero(mocker, get_update_context):
    update, context = get_update_context
    query = CallbackQuery(data=ZERO, id=1, from_user=update.message.from_user, chat_instance='', message=update.message)
    update = Update(1, message=update.message, callback_query=query)
    context.user_data['keyboard_state'] = [['.' for _ in range(3)] for _ in range(3)]
    mock_reply_text = mocker.patch.object(Message, 'reply_text', return_value=None)
    res = await initial_step(update, context)
    count_cross = ''.join([''.join(row) for row in context.user_data['keyboard_state']]).count(CROSS)
    assert count_cross == 1
    assert res == 1


def test_won_cross_row():
    state = [[CROSS for _ in range(3)], [FREE_SPACE for _ in range(3)], [CROSS, ZERO, ZERO]]
    assert won(state, CROSS) == True
    assert won(state, ZERO) == False
...
def test_won_zero_diagonal():
    ...
    for i in range(3):
        state[i][2 - i] = ZERO
    assert won(state, ZERO) == True
    assert won(state, CROSS) == False
...
async def test_game_cross_continue(mocker, get_update_context):
    ...
    query = CallbackQuery(data=(0, 1), id=1, from_user=update.message.from_user, chat_instance='', message=update.message)
    update = Update(1, message=update.message, callback_query=query)
    mock_reply_text = mocker.patch.object(Message, 'reply_text', return_value=None)
    res = await game(update, context)
    assert res == 1
    joined_data = ''.join([''.join(row) for row in context.user_data['keyboard_state']])
...
async def test_game_zero_won(mocker, get_update_context):
    ...
    context.user_data['player_symbol'] = ZERO
    context.user_data['keyboard_state'] = state
    query = CallbackQuery(data=(0, 0), id=1, from_user=update.message.from_user, chat_instance='', message=update.message)
    update = Update(1, message=update.message, callback_query=query)
    mock_reply_text = mocker.patch.object(Message, 'reply_text', return_value=None)
    res = await game(update, context)
    assert res == -1
...
async def test_game_zero_draw(mocker, get_update_context):
    state = [[CROSS, FREE_SPACE, CROSS], [CROSS, ZERO, CROSS], [ZERO, CROSS, ZERO]]
    update, context = get_update_context
    context.user_data['player_symbol'] = ZERO
    context.user_data['keyboard_state'] = state
    query = CallbackQuery(data=(0, 1), id=1, from_user=update.message.from_user, chat_instance='', message=update.message)
